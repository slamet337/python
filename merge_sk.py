...
def proses_mail_merge_sk():
    ...
    excel_path = base_dir / "SK.xlsx"
    template_path = base_dir / "SK1111.docx"  # Sesuaikan dengan nama template Word sertifikat kamu
    output_folder = base_dir / "output_sertifikat_sk"
    ...
    print("=" * 60)
    print(f"Ditemukan {len(df_peserta)} data peserta dari SK.xlsx. Memulai Mail Merge...")
    print("=" * 60)
    ...
    for idx, row in enumerate(df_peserta.itertuples(), 1):
        ...
        clean_seksi = "".join(c for c in seksi if c.isalnum() or c in (' ', '_', '-')).strip()
    ...
    print("\n" + "=" * 60)
    print(f"SELESAI! Semua ({len(df_peserta)}) sertifikat berhasil diproses di folder 'output_sertifikat_sk'.")
    print("=" * 60)
